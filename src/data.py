...
class DataBuilder:
	...
	@staticmethod
	def get_files(data_dir, train=True, raw=False):

		if train:
			train_user_files = pd.read_csv(f"{data_dir}/speech_commands/train/testing_list.txt", header=None).values.flatten()
			train_files_path, train_labels = [], []
			for tr_uf in train_user_files:
				train_files_path.append(f"{data_dir}/speech_commands/train/audio/{tr_uf}")
				label = tr_uf.split("/")[0]
			...
			test_user_files = pd.read_csv(f"{data_dir}/speech_commands/train/validation_list.txt", header=None).values.flatten()
			test_files_path, test_labels = [], []
			# Create dataset files.
			for ts_uf in test_user_files:
				test_files_path.append(f"{data_dir}/speech_commands/test/audio/{ts_uf}")
